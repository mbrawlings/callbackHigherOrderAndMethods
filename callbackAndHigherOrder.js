...
// CODE HERE
// const last = (arr, printLastItem) => printLastItem(arr[arr.length-1])

function last1(arr, printLastItem) {
  printLastItem(arr[arr.length-1])
}
...
// last(names, retrievedLastItem => {
//   console.log('The last name in names is ' + retrievedLastItem)
// })

last1(names, function printLastItem(item) {
  console.log('The last name in names is ' + item)
})
...
// CODE HERE 
const getUserById = (arr, id, nestedFunction) => {
  for (let i = 0; i < arr.length;i++) {
    if (arr[i].id === id) {
      return nestedFunction(arr[i])
    }
  }
}
